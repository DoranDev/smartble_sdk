...
public class SwiftSmartbleSdkPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, BleHandleDelegate, BleScanDelegate, BleScanFilter {
    ...
    func stringReplacingOccurrencesOfString(_ str: String) -> String {
        ...
        let doNotWant = CharacterSet(charactersIn: charactersInString)
...
//     var bg_cal = false
    ...
    var pointerHourImageSize: [UInt32] = .init()
    var pointerMinImageSize: [UInt32] = .init()
    var pointerImageSize: [UInt32] = .init()

    var timeAMImageSize: [UInt32] = .init()

    var timeDateHourImageSize: [UInt32] = .init()
    var timeDateSymbolImageSize: [UInt32] = .init()
    var timeDateMinImageSize: [UInt32] = .init()

    var timeWeekMonthImageSize: [UInt32] = .init()
    var timeWeekSymbolImageSize: [UInt32] = .init()
    var timeWeekDayImageSize: [UInt32] = .init()
    var timeWeekImageSize: [UInt32] = .init()

    var stepImageSize: [UInt32] = .init()
    var hrImageSize: [UInt32] = .init()
    ...
    var pointerBuffer: Data = .init()

    var timeAMBuffer: Data = .init()

    var timeDateHourBuffer: Data = .init()
    var timeDateSymbolBuffer: Data = .init()
    var timeDateMinBuffer: Data = .init()

    var timeWeekMonthBuffer: Data = .init()
    var timeWeekSymbolBuffer: Data = .init()
    var timeWeekDayBuffer: Data = .init()
    var timeWeekBuffer: Data = .init()

    var stepBuffer: Data = .init()
    var hrBuffer: Data = .init()
    var disBuffer: Data = .init()
    var calBuffer: Data = .init()
    ...
    var bleKey: BleKey = .NONE
    var bleKeyFlag: BleKeyFlag = .NONE
...
//    var valueColor = 0
    ...
    var isColor = false
    var dialAssetsFromFlutter = [String: [UInt8]]()
    var pointerModel = 0
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "customDials":
            ...
            dialAssetsFromFlutter = [String: [UInt8]]()
            let assets = args?["assets"] as! [String: Any]
...
//              }
            ...
            case BleKey.AGPS_FILE:
                ...
                if bleKeyFlag == BleKeyFlag.UPDATE {
                    let fileURL = BleCache.shared.mAGpsFileUrl
...
//
            // BleCommand.IO
            case BleKey.WATCH_FACE, BleKey.FONT_FILE, BleKey.UI_FILE, BleKey.LANGUAGE_FILE:
                if bleKeyFlag == BleKeyFlag.DELETE {
                    _ = BleConnector.shared.sendData(bleKey, bleKeyFlag)
                    return
                }
                if bleKeyFlag == BleKeyFlag.UPDATE {
                    var fileURL = args?["url"] as? String
                    ...
                    if path != nil {
                        ...
                        if isExist {
                            ...
                            print("File Error: File not found at path: \(String(describing: path))")
                        }
                    }
                    return
                }
...
//              self.navigationController?.pushViewController(selectVC, animated: true)
            case BleKey.CONTACT:
                ...
                do {
                    listContact?.forEach { contact in
                        ...
                        userName = String(data: newDate, encoding: .utf8) ?? ""

                        if phone.count > 2 {
                            sendArray.append(BleContactPerson(username: userName, userphone: phone))
                        }
                    }
                } catch {
                    bleLog("listContact")
                ...
                let send: BleAddressBook = .init(array: sendArray)
    ...
    func onDeviceFound(_ device: BleDevice) {
        ...
        if let existingIndex = mDevices.firstIndex(where: { ($0 as [String: Any])["deviceMacAddress"] as? String == item["deviceMacAddress"] }) {
            mDevices[existingIndex] = item
        } else {
            mDevices.append(item)
        }

        mDevices.sort { dict1, dict2 -> Bool in
...
extension SwiftSmartbleSdkPlugin {
    ...
    func startCreateBinFile() {
        ...
        minSize = CGSize()
        secSize = CGSize()
        let dataSourceArray = getItemsPiont() as! [String: Any]
...
//            let pvFrame = WatchFaceRect(x: 0, y: 0, width: pvWidth, height: pvHeight)
            ...
            createBinForSupport2DWatchFace(dataSourceArray, isFixCoordinate: isFixCoordinate, bgFrame: bgFrame, pvFrame: pvFrame)
        } else {
            bleLog("isSupp2D is \(isSupp2D)")
            let bgFrame = WatchFaceRect(x: bgX, y: bgY, width: bgWidth, height: bgHeight)
            let pvFrame = WatchFaceRect(x: pvX, y: pvY, width: pvWidth, height: pvHeight)
            // 不支持2D, 如果使用bmp资源图片, 使用下面的方法
            createBinForBMP_File(dataSourceArray, isFixCoordinate: isFixCoordinate, bgFrame: bgFrame, pvFrame: pvFrame)
    ...
    private func createBinForSupport2DWatchFace(_ dataSourceArray: [String: Any], isFixCoordinate: Bool, bgFrame: WatchFaceRect, pvFrame: WatchFaceRect) {
        ...
        let bgByte = viewModel.byteAlignment(newByteBg)
        ...
        let pvByte = viewModel.byteAlignment(newBytePv)
        ...
        for (key, value) in dataSourceArray {
            if key.elementsEqual("TimeAM") {
                ...
                // am
                let ampmRes = viewModel.getImageBufferArray(.AMPM, true, colorNum, true, dialAssetsFromFlutter)

                // hour
                let hourRes = viewModel.getImageBufferArray(.HOUR, true, colorNum, true, dialAssetsFromFlutter)
                // min
                let minRes = viewModel.getImageBufferArray(.MINUTES, true, colorNum, true, dialAssetsFromFlutter)

                // month
                let monthRes = viewModel.getImageBufferArray(.MONTH, true, colorNum, true, dialAssetsFromFlutter)
                let timeWeekMonthSize = monthRes.imageSize
                // day
                let dayRes = viewModel.getImageBufferArray(.DAY, true, colorNum, true, dialAssetsFromFlutter)
                let timeWeekDaySize = dayRes.imageSize
                // week
                let weekRes = viewModel.getImageBufferArray(.WEAK, true, colorNum, true, dialAssetsFromFlutter)
                let timeWeekSize = weekRes.imageSize

                // 日期之间的分割线
                let dateSymbolRes = viewModel.getImageBufferArray(.dateSymbol, true, colorNum, true, dialAssetsFromFlutter)
                let dateSySize = dateSymbolRes.imageSize
                // 星期之间的分割线
                let weekSymbolRes = viewModel.getImageBufferArray(.weakSymbol, true, colorNum, true, dialAssetsFromFlutter)
                ...
                let stepRes = viewModel.getImageBufferArray(.STEP, true, colorNum, false, dialAssetsFromFlutter)
                ...
                let hrRes = viewModel.getImageBufferArray(.HEART_RATE, true, colorNum, false, dialAssetsFromFlutter)
                ...
                let disRes = viewModel.getImageBufferArray(.DISTANCE, true, colorNum, false, dialAssetsFromFlutter)
                ...
                let calRes = viewModel.getImageBufferArray(.CALORIES, true, colorNum, false, dialAssetsFromFlutter)
                ...
                guard let pinitGroup = viewModel.getPointerImage(index, isPNG: true, pointerModel: pointerModel) else {
                    // 这里代表获取指针失败, 需要提示用户
...
//    }
    ...
    func createBinForBMP_File(_ dataSourceArray: [String: Any], isFixCoordinate: Bool, bgFrame: WatchFaceRect, pvFrame: WatchFaceRect) {
        ...
        if isSupp2D {
            ...
            bgByte = viewModel.byteAlignment(newByteBg)
        } else {
            // buffer每个元素大小
        ...
        for (key, value) in dataSourceArray {
            if key.elementsEqual("TimeAM") {
                ...
                // am
                let ampmRes = viewModel.getImageBmpBufferArray(.AMPM, colorNum)
                // hour
                let hourRes = viewModel.getImageBmpBufferArray(.HOUR, colorNum)
                // min
                let minRes = viewModel.getImageBmpBufferArray(.MINUTES, colorNum)

                // month
                let monthRes = viewModel.getImageBmpBufferArray(.MONTH, colorNum)
                let timeWeekMonthSize = monthRes.imageSize
                // day
                let dayRes = viewModel.getImageBmpBufferArray(.DAY, colorNum)
                let timeWeekDaySize = dayRes.imageSize
                // week
                let weekRes = viewModel.getImageBmpBufferArray(.WEAK, colorNum)
                let timeWeekSize = weekRes.imageSize

                // 日期之间的分割线
                let dateSymbolRes = viewModel.getImageBmpBufferArray(.dateSymbol, colorNum)
                let dateSySize = dateSymbolRes.imageSize
                // 星期之间的分割线
                let weekSymbolRes = viewModel.getImageBmpBufferArray(.weakSymbol, colorNum)
                let weekSymSize = weekSymbolRes.imageSize
                ...
                let point: CGPoint = dataSourceArray["Step"] as! CGPoint
                // 获取资源图片相关数据
                let stepRes = viewModel.getImageBmpBufferArray(.STEP, colorNum)
                ...
                let point: CGPoint = dataSourceArray["HR"] as! CGPoint
                let hrRes = viewModel.getImageBmpBufferArray(.HEART_RATE, colorNum)
                ...
                let disRes = viewModel.getImageBmpBufferArray(.DISTANCE, colorNum)
                let disPoint = CGPoint(x: rawDisPoint.x-disRes.imageSize.width, y: rawDisPoint.y)
                ...
                // 卡路里  支持2D  ignoreBlack使用4;
                let calRes = viewModel.getImageBmpBufferArray(.CALORIES, colorNum)
                ...
                guard let pinitGroup = viewModel.getPointerImage(index, isPNG: false, pointerModel: pointerModel) else {
                    // 这里代表获取指针失败, 需要提示用户
...
//
    ...
    func buildWatchFace(_ elements: [Element], _ elementCount: Int, _ imageFormat: Int32) -> BleWatchFaceBin {
        ...
        // header
        bleBin.header = BleWatchFaceBinToHeader(ImageTotal: UInt16(imageCountStart), ElementCount: UInt8(elements.count), ImageFormat: UInt8(imageFormat))
        ...
        for i in 0..<elementCount {
            let newInfo = BleWatchFaceBinElementInfo(imageBufferOffset: 0, imageSizeIndex: imageSizeIndex, w: elements[i].w, h: elements[i].h, x: elements[i].x, y: elements[i].y, imageCount: elements[i].imageCount, type: elements[i].type, gravity: elements[i].gravity, ignoreBlack: elements[i].ignoreBlack, bottomOffset: elements[i].bottomOffset, leftOffset: elements[i].leftOffset, reserved: 0)
            imageSizeIndex += UInt16(elements[i].imageCount)
            bleBin.ElementInfo.append(newInfo)
        }

        // uint32_t[] 所有图片的长度
        var elementImageBufferOffset = UInt32(BleWatchFaceBinToHeader.ITEM_LENGTH+infoSize+(4 * imageCountStart))
...
//        }
    ...
    func getImagePathToData(_ imageName: String, ofType: String) -> Data {
        ...
        if ofType.elementsEqual("bmp") {
            ...
            let bmpWidth = Int(newImage!.size.width) // 图片宽度
            let bitCount = 16 // 位深度，可为8，16，24，32
            ...
            var image16: Data = NSData(contentsOfFile: String(newPath!))! as Data
            // 删除BMP头部信息
            let headerInfoSize = Int(image16[10]) // 头部信息长度
            for _ in 0..<headerInfoSize {
                image16.remove(at: 0)
            }
            // 判断单、双数，单数要减去无用字节
            let dataCount: Int = image16.count/rowSize
            let tmpNewData = NSMutableArray()
            let imageByte = [UInt8](image16)
    ...
    func getMainBgImageNew() -> UIImage {
        ...
        if isSquare == false {
            if BleCache.shared.mWatchFaceType == BleDeviceInfo.WATCH_FACE_320x363 {
                ...
                if newData!.count > 10 {
                    newImage1 = UIImage(data: newData!)
                }
            }
        }
