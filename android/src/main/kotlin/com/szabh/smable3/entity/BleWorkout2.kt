...
data class BleWorkout2(
    ...
    var mMinBpm: Int = 0,       //最小心率
    var mUndefined: Int = 0,    //占位用未定义，字节对齐预留
    var mMaxSpm: Int = 0,       //最大步频，单位：步数每分钟
    var mMinSpm: Int = 0,       //最小步频，单位：步数每分钟
    var mMaxPace: Int = 0,      //最大(最慢)配速，单位：秒每公里
    var mMinPace: Int = 0       //最小(最快)配速，单位：秒每公
) : BleReadable() {
    ...
    override fun decode() {
        ...
        mMinBpm = readInt8().toInt()
        mUndefined = readInt8().toInt()
        mMaxSpm = readInt16().toInt()
        mMinSpm = readInt16().toInt()
        mMaxPace = readInt32()
        mMinPace = readInt32()
    }

    companion object {
        const val ITEM_LENGTH = 128
    }
}
